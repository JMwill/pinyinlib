// Thanks to cute-jumper

// ASCII char to simplifed Chinese characters.
const SIMPLIFIED_CHAR_TABLE = [
...
export function buildRegexpChar(char: string, noPuncP?: boolean, traditionalP?: boolean, onlyChineseP?: boolean, mixedP?: boolean) {
  const charCodeOfa = 97;
  const charCode = char.charCodeAt(0);
  const diff = charCode - charCodeOfa;
  let regexp;
  const c = String.fromCodePoint(charCode);
  if (diff > 25 || diff < 0) {
    return (!noPuncP && PUNCTUATION_ALIST[c]) || c;
  } else {
    regexp = mixedP
      ? `${TRADITIONAL_CHAR_TABLE[diff]}${SIMPLIFIED_CHAR_TABLE[diff]}`
      : (traditionalP ? TRADITIONAL_CHAR_TABLE[diff] : SIMPLIFIED_CHAR_TABLE[diff]);
...
export function buildRegexpString(str: string, noPuncP?: boolean, traditionalP?: boolean, onlyChineseP?: boolean, mixedP?: boolean) {
  let result = '';
  for (const c of str) {
    result += buildRegexpChar(c, noPuncP, traditionalP, onlyChineseP, mixedP);
  }
  return result;
}
